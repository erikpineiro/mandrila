State {
	...
	home: {
		dbStatus
		choosingLevel { 
			// Used when user chooses specific level for drill
			// choosingLevel = Nul means not choosing level
			textSample
			selectedLevel // -1: not choosing, 0: choosing but no level selected, 1: level1, etc.
		}
	}
	drilling: {
		readingID //of class reading // Null => No drill going on
		...
		reporting // index of report being shown (-1: not reporting). Always related to the selected exercise
	}
	racing: {
		// Null => No race going on
		race // of class race
	...
	// NORMALIZED DATA
	exercises: {
		// Each exercise is a property in the object
		// the exerciseID is the key
		// Keep them here instead of nested in languages. Normalized.
		// So if change to one, change to all
		key: value
	}
	readings: {
		// Readings that have already happened (not current drill / race).
		// Each reading a property in the object
		// the readingID is the key
		// Keep them here instead of nested in languages. Normalized.
		// So if change to one, change to all
		key: value
	}
...
Actions {
	APP_DRILL_OPEN(exercise) : _PENDING, _REJECTED, _FULFILLED // Also from New! after Drill Review
	...
	APP_SELECT_LANGUAGE(language) // Changes current language
	APP_USER_ CHOOSE() // Show page that lets user go to any settings-page
	APP_USER_ CLOSE()

	DRILL_CANCEL()
	DRILL_CHOOSELEVEL_OPEN()
	DRILL_CHOOSELEVEL_CLOSE()
	DRILL_CHOOSELEVEL_SHOWEXAMPLE(level) : _PENDING, _REJECTED, _FULFILLED
	DRILL_DONE() : _PENDING, _REJECTED, _FULFILLED // After Review. This is were Rating is passed to DB. 
	DRILL_PUT_MARK(nStatement, markedAs)
	...
	EXERCISE_SHOW_REPORT (index) // Creates report if index > #reports
	EXERCISE_CLOSE_REPORT() : _PENDING, _REJECTED, _FULFILLED
	EXERCISE_REPORT(report) : _PENDING, _REJECTED, _FULFILLED

	HOME_OPEN_LEVEL_CHOOSER()

	RACE_CLOSE()
	RACE_OPEN(clase) : _PENDING, _REJECTED, _FULFILLED
	// RACE_OPEN_TOGGLEREMINDER()
	RACE_SELECT_SCENE(nScene)
	RACE_PUT_MARK(nScene, nStatement, markedAs)
	...
	RACE_SET_TIMER() // params: "-1",one second less "x" set to x seconds
									 // Update opponents here instead of own action (RACE_PUT_OPPONENTS() : _PENDING, _REJECTED, _FULFILLED)
	RACE_PUT_ONEMARK(drillNr, statementNr, mark, result) : _PENDING, _REJECTED, _FULFILLED
	...
	USER_ CLASE_CHANGE(language, newClase) : _PENDING, _REJECTED, _FULFILLED // Makes the change in users DB
	USER_ LANGS_ADD(language) : _PENDING, _REJECTED, _FULFILLED
	USER_ LANGS_DELETE(language) : _PENDING, _REJECTED, _FULFILLED
	USER_ LANGS_SETAPPLANG (language) : _PENDING, _REJECTED, _FULFILLED
	USER_ OPEN (which)
	USER_ TEACHERS_ADD(userID) : _PENDING, _REJECTED, _FULFILLED
	USER_ TEACHERS_DELETE(userID) : _PENDING, _REJECTED, _FULFILLED
	USER_ TEACHERS_SEARCH(string) : _PENDING, _REJECTED, _FULFILLED
	USER_ TEACHERS_SELECT(userID)
	USER_ INFO_ALIAS_WRITTEN(alias) : _PENDING, _REJECTED, _FULFILLED
	USER_ INFO_SAVE(userInfo) : _PENDING, _REJECTED, _FULFILLED
}
